...
class TTFontFile:
    ...
    def read_tag(self):
        self._pos += 4
        return frombytes(self.fh.read(4))
    ...
    def add(self, tag, data):
        if (tag == 'head') :
            data = self.splice(data, 8, tobytes("\0\0\0\0"))        
...
############################################/
    ...
    def makeSubset(self, file, subset):
        ...
        post = tobytes("\x00\x03\x00\x00") + substr(opost,4,12) + tobytes("\x00" * 16)
        self.add('post', post)
